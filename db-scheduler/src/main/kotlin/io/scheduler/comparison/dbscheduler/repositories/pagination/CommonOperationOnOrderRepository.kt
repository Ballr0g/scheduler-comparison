...
import io.scheduler.comparison.dbscheduler.domain.OrderOperationStatus
...
class CommonOperationOnOrderRepository(
    ...
    private companion object {
        ...
        const val READ_UNPROCESSED_ORDER_OPERATIONS_FOR_UPDATE_SQL =  """
                ...
                FROM db_scheduler.order_statuses
                WHERE
        ...
        const val UPDATE_SUCCESSFULLY_HANDLED_ORDER_OPERATION_SQL = """
            UPDATE db_scheduler.order_statuses
            SET
    ...
    fun readUnprocessedOrders(
        maxPageSize: Int,
        orderTaskState: TaskState<CommonOrderTaskData, CommonOrderTaskMetadata>
    ): List<OperationOnOrder> {
        val taskData = orderTaskState.taskData
        return jdbcClient.sql(READ_UNPROCESSED_ORDER_OPERATIONS_FOR_UPDATE_SQL)
            .param("maxPageSize", maxPageSize)
            .param("excludedMerchantIds", taskData.excludedMerchantIds.toTypedArray())
            .param("orderStatuses", taskData.orderStatuses.asSequence().map { it.value }.toSet())
